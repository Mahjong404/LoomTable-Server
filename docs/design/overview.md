...
Server、PostgreSQL 和文件卷由 Docker Compose 部署。Redis、消息队列和对象存储属于后续 Team 或远程生产部署能力。
...
- Mutation 具有幂等性。
- Schema 变更不会静默丢失数据。
- Server 重启后数据和 Managed Attachment 引用保持一致。

## 7. 许可证

LoomTable Server 使用 AGPL-3.0。第三方依赖、文件存储、地图服务和其他外部组件需要单独审核许可证和部署条款。
