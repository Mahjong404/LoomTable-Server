...
- Distinct Values（`values/query`）与记录聚合（`records/aggregate`）复用同一查询投影和 Filter 编译；`unfilteredTotal` 只在首页面与 `totalCount` 同事务计算。Record Move 通过取锚点中点只写一行 `position`，不做全表重编号。
